  ...
  <div class="min-h-screen bg-surface-dark text-gray-200 font-mono">
    <header class="border-b border-border px-6 py-4 flex items-center justify-between">
      <div class="flex items-center gap-6">
        ...
        <!-- Reserve width so plugin router-links arriving async from
             /api/plugins/list don't slide the project selector right. -->
        <nav class="flex gap-1 min-w-[180px]">
          <router-link
            to="/dashboard"
          ...
          >Tunnels</router-link>
          <router-link
            v-if="features.resolveEnabled"
            to="/repro"
            class="px-3 py-1 text-sm rounded transition-colors"
            :class="route.path.startsWith('/repro')
              ? 'bg-surface text-white'
              : 'text-gray-500 hover:text-gray-300'"
          >Repro</router-link>
          <router-link
