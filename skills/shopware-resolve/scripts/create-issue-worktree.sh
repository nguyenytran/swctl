...
try_swctl() {
  ...
  if swctl create "${swctl_args[@]}"; then
    ...
    meta_file=$(find ~/.local/state/swctl/instances -name "${ISSUE_NUMBER}.env" 2>/dev/null | head -1)
    if [[ -n "$meta_file" ]]; then
      local wt_path plugin_wt_path domain swctl_mode
      wt_path=$(grep '^WORKTREE_PATH=' "$meta_file" | cut -d= -f2)
      plugin_wt_path=$(grep '^PLUGIN_WORKTREE_PATHS=' "$meta_file" | cut -d= -f2)
      domain=$(grep '^DOMAIN=' "$meta_file" | cut -d= -f2)
      swctl_mode=$(grep '^SWCTL_MODE=' "$meta_file" | cut -d= -f2)

      # Post-provisioning: activate plugin and rebuild bundles.
      # QA mode syncs pre-built assets from trunk but does NOT activate plugins.
      # Without this, the plugin's admin/storefront UI won't appear.
      if [[ -n "$PLUGIN_NAME" ]]; then
        echo "Activating plugin: $PLUGIN_NAME"
        swctl exec "$ISSUE_NUMBER" \
          "php bin/console plugin:refresh --quiet && php bin/console plugin:install --activate $PLUGIN_NAME" \
          2>&1 | tail -3 || echo "  [warn] plugin activation failed"

        echo "Rebuilding bundles..."
        swctl exec "$ISSUE_NUMBER" \
          "php bin/console bundle:dump --quiet && php bin/console assets:install --quiet && php bin/console cache:clear --quiet" \
          2>&1 | tail -3 || echo "  [warn] bundle rebuild failed"
      fi
