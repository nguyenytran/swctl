...
const projectSlugs = computed(() => {
  const slugs = new Set(instances.value.map(i => i.projectSlug).filter(Boolean))
...
function bulkDelete() {
  ...
  confirmAction.value = {
    ...
    message: `Delete ${ids.length} worktree(s)? This cannot be undone.`,
    onConfirm: async () => {
      for (const id of ids) {
        stream.start(buildStreamUrl('clean', { issueId: id }))
        // Wait a bit between deletes to avoid stream conflicts
      }
      selected.value = new Set()
...
<template>
  <div>
    ...
    <div v-else-if="!loading" class="text-center py-16 text-gray-500">
      <p v-if="instances.length === 0" class="text-lg mb-2">No worktrees</p>
