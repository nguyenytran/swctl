...
@test "preflight: skips docker network check under OrbStack" {
    ...
    # mask the network skip we're testing.  `docker info`, alpine image,
    # and a present compose template are forced-passing.
    docker() {
        case "$1" in
            info)             return 0 ;;
            image)            return 0 ;;
    ...
    docker() {
        case "$1" in
            ...
            network)
                ...
                printf '__NETWORK_INSPECT_CALLED__\n' >> "$SW_TMP/calls.log"
                # Return 0 so the check passes; we're testing it ran,
                # not its outcome
                return 0
                ;;
        esac
